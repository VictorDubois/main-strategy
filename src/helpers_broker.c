...
float compute_m_linear_speed(const long encoder1, const long encoder2, const long elapsed)
{
    ...
    speed1 = (DIST_PER_REVOLUTION * (float)diff_encoder1 / TICKS_PER_REVOLUTION)
             / (float)(elapsed / 1e9);
    speed2 = (DIST_PER_REVOLUTION * (float)diff_encoder2 / TICKS_PER_REVOLUTION)
             / (float)(elapsed / 1e9);
    ...
    return speed; // convert to meters
}
